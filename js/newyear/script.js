...
const IS_HIGH_END_DEVICE = (() => {
	...
	const minCount = window.innerWidth <= 1024 ? 4 : 8; // 计算机使用8核，其余4
...
const store = {
	...
	state: {  
		...
		//用于<select>的config值必须是字符串，除非手动更改
		...
		config: {
			...
			shell: 'Random',//烟花类型
			size: IS_DESKTOP//缩放
				? '3' // Desktop default 台式机默认
				: IS_HEADER 
					? '1.2' // Profile header default (doesn't need to be an int)
					: '2', // Mobile default
			autoLaunch: true,//自动发射烟花
			finale: true,//
			skyLighting: SKY_LIGHT_NORMAL + '',//是否照亮天空
			hideControls: IS_HEADER,//隐藏按钮
			longExposure: false,//长曝光（保留烟花火花）
...
const helpContent = {
	shellType: {
		header: '烟花类型',
		body: '你要放的烟花的类型。选择“随机（Random）”可以获得更好的体验！'
	},
	shellSize: {
		header: '烟花大小',
...
const nodeKeyToHelpKey = {
	shellTypeLabel: 'shellType',//烟花类型
	shellSizeLabel: 'shellSize',//大小
	qualityLabel: 'quality',//画质
	skyLightingLabel: 'skyLighting',//天空照亮
	scaleFactorLabel: 'scaleFactor',//?
	autoLaunchLabel: 'autoLaunch',//自动发射
	finaleModeLabel: 'finaleMode',
	hideControlsLabel: 'hideControls',//隐藏按钮
	fullscreenLabel: 'fullscreen',//全屏
	longExposureLabel: 'longExposure'//长曝光
};
